  ...
  const renderColumns = columns.map(({ name, align, width }, key) => {
    ...
    return (
      <SoftBox
        ...
        color="secondary" opacity={0.7} borderBottom={`${borderWidth[1]} solid ${light.main}`}>
        {name.toUpperCase()}
      </SoftBox>
    );
  });
  ...
  const renderRows = rows.map((row, key) => {
    ...
    const tableRow = columns.map(({ name, align }) => {
      ...
      if (Array.isArray(row[name])) {
        template = (
          <SoftBox key={uuidv4()} component="td" p={1} borderBottom={row.hasBorder ? `${borderWidth[1]} solid ${light.main}` : null}>
